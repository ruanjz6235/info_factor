...
class BaseGenerator:
    def __init__(self, sentence, dates):
        """"""
        self.dates = dates
    ...
    def stock_df(self):
        if self.wencai_data:
            return query_iwencai(self.sentence)
        else:
            return getattr(self.api, self.sentence)(*self.dates)
...
class GQJL_HIS(BaseGenerator):
    def __init__(self, dates):
        ...
        self.stock_df = self.cal_raw_data(self.stock_df)

    def cal_raw_data(self, stock_df, **kwargs):
        stock_df.columns = ['stock_code', 'declare_date', 'jlzs_ratio', 'xq_price']
        stock_df['declare_date'] = stock_df[['stock_code', 'declare_date']].apply(self.get_tradeday, axis=1)
        return stock_df.groupby(['stock_code', 'declare_date']).agg({'jlzs_ratio': 'sum', 'xq_price': 'mean'})
...
class STK313(BaseGenerator):
    ...
    def cal_raw_data(self, stock_df, **kwargs):
        ...
        stock_df.columns = ['stock_code', 'declare_date', 'yjzb']
        stock_df['declare_date'] = stock_df[['stock_code', 'declare_date']].apply(self.get_tradeday, axis=1)
        return stock_df
...
class GQJL:
    def __init__(self, dates):
        ...
        gqjl_his.stock_df = gqjl_his.cal_raw_data(gqjl_his.stock_df)
        stk313 = STK313(dates)
        stk313.stock_df = stk313.cal_raw_data(stk313.stock_df)
        self.stock_df = gqjl_his.stock_df.merge(stk313.stock_df, on=['stock_code', 'declare_date'], how='left')
...
class GFHG(BaseGenerator):
    ...
    def map_data(self, x, *args):
        ...
        if x[2] / x[10] > x01:
            hgjg = y01
        elif (x[2] / x[10] >= x02) & (x[2] / x[10] <= x01):
            hgjg = y02
        else:
            hgjg = y03
...
class FHPX(BaseGenerator):
    ...
    def map_data(self, x, *args):
        ...
        if x[3] > 0.02:
            fhbl = 1.5
        elif (x[3] <= 0.02) & (x[3] >= 0.005):
            fhbl = 0.5
        elif (x[3] < 0.005) & (x[3] > 0):
            fhbl = 0.1
        else:
            fhbl = 0
...
class YJBG(BaseGenerator):
    def __init__(self, dates):
        ...
        :param dates:
        """
        super(YJBG, self).__init__(sentence='get_yjbg', dates=dates)
        self.wencai_data = False
        self.col = ['thscode', 'fhl', 'szl', 'f036v_stk441', 'f037d_stk441']
...
class LADC(BaseGenerator):
    ...
    def cal_raw_data(self, stock_df, **kwargs):
        return stock_df.groupby(self.col[:2])['立案调查对象'].agg(''.join).reset_index()
...
class NBFB(BaseGenerator):
    def __init__(self, dates):
        ...
        self.wencai_data = False
        self.col = ['thscode', 'declaredate', 'f003', 'nbfb_score']

    def cal_raw_data(self, stock_df, **kwargs):
        return stock_df[stock_df['declaredate'].notnull()]
...
class PXDM(BaseGenerator):
    def __init__(self, dates):
        ...
        :param dates:
        """
        super(PXDM, self).__init__(sentence='get_djzy', dates=dates)
        self.wencai_data = False
        self.col = ['thscode', 'declaredate_stk019', 'f005n_stk019', 'nbfb_score']

    def cal_raw_data(self, stock_df, **kwargs):
        return stock_df[stock_df['declaredate_stk019'].notnull()]
...
class WGCF(BaseGenerator):
    ...
    def cal_raw_data(self, stock_df, **kwargs):
        return stock_df[stock_df['discdate_stk656'].notnull()]
