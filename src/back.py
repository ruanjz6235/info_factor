...
def backtest(score, ret, index_ret, name, k=0, decay='decay', labels=[]):
    """
    回测
    :param labels:
    :param score:
    ...
    for m in [1, 3, 5, 10, 20]:  # 一日、三日、五日、十日、二十日持仓
        ...
        for i in range(int(x), int(y)+1):  # 0-1, 1-2, 2-3, 3-4, 4-5
            ...
            if len(labels) > 0:
                labels_i = [str(j) for j in labels[i]]
                single = {'_'.join(labels_i) + '-' + str(i) + '-' + str(m): cum(asset_ret),
                          '_'.join(labels_i) + '-' + str(i) + '-' + str(m) + '-count': asset_count}
            else:
                single = {str(i) + '-' + str(m): cum(asset_ret),
                          str(i) + '-' + str(m) + '-count': asset_count}
            sub_asset_cum.update(single)
        ...
        asset_cum.update(sub_asset_cum.copy())
        sub_asset_cum.update({'index': cum(index_ret)})
